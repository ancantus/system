...
private:
    ...
    char const* category_name() const noexcept
    {
        ...
        {
            const error_category& ec = *reinterpret_cast<error_category const*>(cat_);
            return ec.name();
        }
    }
...
public:
    ...
    template<class ErrorCodeEnum> BOOST_SYSTEM_CONSTEXPR error_code( ErrorCodeEnum e,
        ...
        >::type* = 0 ) noexcept: val_(), cat_(), lc_flags_( 0 )
    {
    ...
    BOOST_SYSTEM_CONSTEXPR int value() const noexcept
    {
        ...
        else
        {
            ...
            unsigned ch = static_cast<unsigned>( reinterpret_cast<boost::uintptr_t>( cat_ ) % 2097143 ); // 2^21-9, prime
    ...
    BOOST_SYSTEM_CONSTEXPR const error_category & category() const noexcept
    {
        ...
        {
            return *reinterpret_cast<error_category const*>(cat_);
        }
    }
    ...
    std::string message() const
    {
        if( lc_flags_ == 1 )
        {
            std::error_category const& category = *reinterpret_cast<std::error_category const*>( cat_ );
            return category.message( val_ );
        }
        else if( lc_flags_ == 0 )
        {
    ...
    char const * message( char * buffer, std::size_t len ) const noexcept
    {
        if( lc_flags_ == 1 )
        {
            ...
            {
                const std::error_category& cat = *reinterpret_cast<std::error_category const*>(cat_);
                detail::snprintf( buffer, len, "No message text available for error std:%s:%d", cat.name(), val_ );
                return buffer;
            }
#endif
        }
...
public:
    ...
    BOOST_SYSTEM_CONSTEXPR inline friend bool operator==( const error_code & lhs, const error_code & rhs ) noexcept
    {
        ...
        if ( s1 && s2 )
        {
            std::error_category const& lhs_cat = *reinterpret_cast<std::error_category const*>( lhs.cat_ );
            std::error_category const& rhs_cat = *reinterpret_cast<std::error_category const*>( rhs.cat_ );

            return lhs.val_ == rhs.val_ && lhs_cat == rhs_cat;
        }
        else
        {
    ...
    BOOST_SYSTEM_CONSTEXPR inline friend bool operator<( const error_code & lhs, const error_code & rhs ) noexcept
    {
        ...
        if( s1 && s2 )
        {
            ...
            return lhs_cat < rhs_cat || (lhs_cat == rhs_cat && lhs.val_ < rhs.val_);
        }
        else
        {
    ...
    operator std::error_code () const
    {
        if( lc_flags_ == 1 )
        {
            return std::error_code( val_, *reinterpret_cast<std::error_category const*>(cat_));
        }
        else if( lc_flags_ == 0 )
        {
// This condition must be the same as the one in error_category_impl.hpp
        ...
        {
            return std::error_code( val_, category() );
        }
    }
